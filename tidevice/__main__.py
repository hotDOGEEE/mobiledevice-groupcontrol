...
def cmd_install_udid(udid, params):
    """
    给群控用的，就少搞点参数，每个设备装完包的事件必定是不一样的，所有设备都装好了，手动给一个启动吧
    Args:
        udid: 指定的设备
        params.filepath_or_url: 文件路径或者url，看都看得懂

    Returns:

    """
    d = _udid2device(udid)
    bundle_id = d.app_install(params.filepath_or_url)
    return bundle_id
...
def cmd_uninstall_udid(udid, params):
    d = _udid2device(udid)
    ok = d.app_uninstall(params.bundle_id)
    if ok:
        return True
...
def cmd_applist_udid(udid, params):
    d = _udid2device(udid)
    list_installed = list()
    for info in d.installation.iter_installed():
        bundle_id = info["CFBundleIdentifier"]
        try:
            display_name = info["CFBundleDisplayName"]
            version = info.get("CFBundleShortVersionString", "")
            list_installed.append(dict(bundle=bundle_id, disname=display_name, ver=version))
        except BrokenPipeError:
            break
...
def cmd_launch_udid(udid, params):
    d = _udid2device(udid)
    try:
        pid = d.instruments.app_launch(params.bundle_id)
        return pid
    except ServiceError as e:
        sys.exit(e)
...
def cmd_syslog_sock_orient(udid: str):
    d = _udid2device(udid)
    s = d.start_service("com.apple.syslog_relay")
    return s
...
_commands = [
    ...
    dict(action=cmd_perf,
         ...
         flags=[dict(args=['-B', '--bundle_id'], help='app bundle id', required=True),
                dict(args=['-o'], dest='perfs', help='cpu,memory,fps,network,screenshot. separate by ","',
                     required=False), ],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Tool for communicate with iOS devices, version {}, created: codeskyblue 2020/05".format(
            __version__),
